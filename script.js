...
// Close mobile menu when clicking on a link (but not dropdown items)
...
document.querySelectorAll('.dropdown-item').forEach(item => {
    item.addEventListener('click', function(e) {
        // Allow normal navigation for dropdown items
        // Don't prevent default or close dropdown
    });
});
...
// Handle cross-page navigation with smooth transitions (but not dropdown items)
document.querySelectorAll('a[href$=".html"]:not(.dropdown-item)').forEach(link => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    steps.forEach(step => {
        step.addEventListener('mouseenter', () => {
            ...
            step.style.background = 'rgba(168, 85, 247, 0.2)';
        });
        
        step.addEventListener('mouseleave', () => {
            step.style.transform = 'translateX(0)';
            step.style.background = 'rgba(168, 85, 247, 0.1)';
        });
    });
});
...
style.textContent = `
    .nav-link.active {
        ...
        text-shadow: 0 0 5px #a855f7 !important;
    }
